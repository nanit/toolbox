...
%SHAKED
% MAX_NUM_OF_BBS = 1e5;
% bbs = bbs(1:min(end,MAX_NUM_OF_BBS),:);
%/SHAKED
n=size(bbs,1);
kp = true(1,n);
area=bbs(:,3).*bbs(:,4);
...
for i = 1:n
    ...
    for j = (i+1):n
        ...
        xIntersection=min(xEnd(i),xEnd(j))-max(xStart(i),xStart(j));
        if(xIntersection<=0)
            continue;
        end
        yIntersection=min(yEnd(i),yEnd(j))-max(yStart(i),yStart(j));
        if(yIntersection<=0)
            continue;
        end
        currOverlap = xIntersection * yIntersection;
        if(ovrDnm)
            unionArea=area(i)+area(j)-currOverlap;
        else
            unionArea=min(area(i),area(j));
        end
        currOverlap=currOverlap/unionArea;
